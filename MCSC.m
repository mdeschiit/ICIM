...
%@Date:     1/6/2021
%@Project:  Sensitivity Analysis on C Monte Carlo Simulation
...
trGerbermin = zeros(21,1);
trGerbermax = zeros(21,1);
for k = 1:21

    cfrac = (k-1)*0.1;
    %Calculate interval C bounds
    ClGerberIP = exp(b0Gerber+min(eGerber)*cfrac);
    CuGerberIP = exp(b0Gerber+max(eGerber)*cfrac);
