...
SIntL = SmaxL - SminU;
SIntU = SmaxU - SminL;
% SaIntL = (SmaxL-SminU)/2;
% SaIntU = (SmaxU-SminL)/2;
% SmIntL = (SmaxL+SminL)/2;
% SmIntU = (SmaxU+SminU)/2;
% SapIntGerberL = min([(SmaxL-SminL)./(2-2.*((SmaxL-SminL)./(2*Su))) (SmaxL-SminU)./(2-2.*((SmaxL-SminU)./(2*Su))) (SmaxU-SminL)./(2-2.*((SmaxU-SminL)./(2*Su))) (SmaxU-SminU)./(2-2.*((SmaxU-SminU)./(2*Su)))],[],2);
% SapIntGerberU = max([(SmaxL-SminL)./(2-2.*((SmaxL-SminL)./(2*Su))) (SmaxL-SminU)./(2-2.*((SmaxL-SminU)./(2*Su))) (SmaxU-SminL)./(2-2.*((SmaxU-SminL)./(2*Su))) (SmaxU-SminU)./(2-2.*((SmaxU-SminU)./(2*Su)))],[],2);
...
ADTT = 200;
DdIntGerberLIP = ADTT*min([sum(SpIntGerberL.^mIP)/ClIP sum(SpIntGerberU.^mIP)/ClIP sum(SpIntGerberL.^mIP)/CuIP sum(SpIntGerberU.^mIP)/CuIP]);
DdIntGerberUIP = ADTT*max([sum(SpIntGerberL.^mIP)/ClIP sum(SpIntGerberU.^mIP)/ClIP sum(SpIntGerberL.^mIP)/CuIP sum(SpIntGerberU.^mIP)/CuIP]);
disp("Interval Damage (IP): [ "+DdIntGerberLIP+", "+DdIntGerberUIP+" ]");
...
%Mean Damage
DmIntGerberLIP = DdIntGerberLIP/td;
DmIntGerberUIP = DdIntGerberUIP/td;
